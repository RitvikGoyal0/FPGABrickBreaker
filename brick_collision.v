module brick_collision #(
  ...
  always @(*) begin
    ...
    if (brick_alive[row_d * NUM_COLS + col_d] && (colliding_edge_d_y >= brick_top_d_y) && (colliding_edge_d_y <= brick_top_d_y + BRICK_HEIGHT) && (colliding_edge_d_x >= brick_left_d_x) && (colliding_edge_d_x <= brick_left_d_x + BRICK_WIDTH)) begin
      ...
      new_ball_dx          = -ball_dx;
      new_ball_dy          = -ball_dy;
    end else if(brick_alive[row_x * NUM_COLS + col_x] && (potent_pos_y >= brick_top_x_y) && (potent_pos_y <= brick_top_x_y + BRICK_HEIGHT) && (colliding_edge_x >= brick_left_x_x) && (colliding_edge_x <= brick_left_x_x + BRICK_WIDTH)) begin
